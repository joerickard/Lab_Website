...
function loadStatsPage() {
	...
	for (var i = 2; i < table.rows.length; i++) {
		...
		if (row.cells[2].innerHTML > row.cells[3].innerHTML) {
			row.cells[4].innerHTML = "Colorado";
		}
		else {
...
function loadPlayersPage() {
	...
	for (var i = 0; i < players.length; i++) {
		parsedPlayers = parsedPlayers + "<a href=\"#\" onclick=switchPlayers(" + i + ")>"+ players[i].name;
	}
	document.getElementById("player_selector").innerHTML = parsedPlayers;
}
...
function switchPlayers(playerNum) {
	...
	document.getElementById("player_img").alt = "../resources/img/helmet.jpg";
